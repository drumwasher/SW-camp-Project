...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(DCmotor_Speed_Ctrl_M, 0);
  ...
  DCmotor_Speed_Ctrl_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(DCmotor_Speed_Ctrl_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(DCmotor_Speed_Ctrl_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(DCmotor_Speed_Ctrl_M);
