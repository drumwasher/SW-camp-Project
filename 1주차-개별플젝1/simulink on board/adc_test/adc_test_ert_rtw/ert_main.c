...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) adc_test_M->Timing.clockTick0;
  adc_test_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(adc_test_M, 0);
  ...
  errorCode = extmodeInit(adc_test_M->extModeInfo, &rtmGetTFinal(adc_test_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(adc_test_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(adc_test_M);
