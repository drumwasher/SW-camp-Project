...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ex2_encoder_M, 0);
  ...
  ex2_encoder_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ex2_encoder_M->extModeInfo, &rtmGetTFinal
    (ex2_encoder_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ex2_encoder_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ex2_encoder_M);
