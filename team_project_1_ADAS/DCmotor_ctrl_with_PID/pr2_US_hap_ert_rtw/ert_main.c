...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(pr2_US_hap_M, 0);
  ...
  pr2_US_hap_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(pr2_US_hap_M->extModeInfo, &rtmGetTFinal(pr2_US_hap_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(pr2_US_hap_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(pr2_US_hap_M);
