...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(motor_ctrl_with_PID_M, 0);
  ...
  motor_ctrl_with_PID_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(motor_ctrl_with_PID_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(motor_ctrl_with_PID_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(motor_ctrl_with_PID_M);
