...
void pr2_US_hap_step(void)
{
...

  {
    ...
    rtb_Integrator1 = pr2_US_hap_X.Integrator1_CSTATE;

    /* Step: '<S6>/Step1' incorporates:
     *  Step: '<S8>/Step1'
     */
    tmp_2 = pr2_US_hap_M->Timing.t[0];
    if (tmp_2 < pr2_US_hap_P.Step1_Time) {
      rtb_Add1 = pr2_US_hap_P.Step1_Y0;
    } else {
      rtb_Add1 = pr2_US_hap_P.Wref;
    }
    ...
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
        ...
        switch (pr2_US_hap_DW.is_c3_pr2_US_hap) {
         ...
         case pr2_US_hap_IN_drive_on_adas_on:
          if (pr2_US_hap_B.TmpRTBAtChartInport2 == 0.0) {
            ...
            pr2_US_hap_B.out1 = 10.0 * pr2_US_hap_DW.steering +
         ...
         case pr2_US__IN_drive_on_adas_on_FUS:
          if ((pr2_US_hap_B.Constant1 <= 10.0) && (pr2_US_hap_B.Constant3 > 10.0))
          {
            ...
            pr2_US_hap_B.out1 = pr2_US_hap_DW.default_k - (0.0 -
              pr2_US_hap_B.Constant1) * pr2_US_hap_DW.steering;
         ...
         default:
          ...
          if (pr2_US_hap_B.Constant1 > 10.0) {
            ...
            pr2_US_hap_DW.is_c3_pr2_US_hap = pr2_US_IN_drive_on_adas_on_LRUS;
            out1_tmp = (0.0 - pr2_US_hap_B.Constant1) * pr2_US_hap_DW.steering;
        ...
        switch (pr2_US_hap_DW.is_c1_pr2_US_hap) {
         ...
         case pr2_US_hap_IN_adas_on:
          if (pr2_US_hap_B.TmpRTBAtChart1Inport1 == 0.0) {
            ...
            pr2_US_hap_B.rout2 = pr2_US_hap_B.rout1;
          } else {
            pr2_US_hap_B.rout1 = pr2_US_hap_B.out1;
            pr2_US_hap_B.rout2 = pr2_US_hap_B.out2;
          }
       ...
       *  Sum: '<S6>/Add2'
       */
      pr2_US_hap_B.Divide1 = (pr2_US_hap_B.rout1 - pr2_US_hap_P.Constant_Value_o)
        / pr2_US_hap_P.Constant1_Value;

      /* Product: '<S6>/Divide3' incorporates:
       *  Constant: '<S6>/Constant2'
       */
      pr2_US_hap_B.Divide3 = pr2_US_hap_B.Divide1 / pr2_US_hap_P.Constant2_Value;

      /* DiscreteTransferFcn: '<S4>/Discrete Transfer Fcn' */
      pr2_US_hap_B.DiscreteTransferFcn = tmp_3;
     ...
     *  Product: '<S6>/Divide2'
     */
    pr2_US_hap_B.Add4 = rtb_Add1 * pr2_US_hap_B.Divide3 + rtb_Add1;

    /* Sum: '<Root>/Add1' incorporates:
     *  DiscreteTransferFcn: '<S4>/Discrete Transfer Fcn'
     */
    rtb_Add1 = pr2_US_hap_B.Add4 - sMultiWord2Double
     ...
     *  Gain: '<S7>/Gain5'
     */
    pr2_US_hap_B.Subtract1 = pr2_US_hap_P.KP * rtb_Add1 + rtb_Integrator1;
     ...
     *  Gain: '<S5>/Gain1'
     */
    rtb_Integrator1 = floor(255.0 / pr2_US_hap_P.Vlim * pr2_US_hap_B.Saturation);
    if (rtIsNaN(rtb_Integrator1) || rtIsInf(rtb_Integrator1)) {
      rtb_Integrator1 = 0.0;
    } else {
      rtb_Integrator1 = fmod(rtb_Integrator1, 256.0);
    }

    /* DataTypeConversion: '<S5>/Data Type Conversion' */
    pr2_US_hap_B.DataTypeConversion_e = (uint8_T)(rtb_Integrator1 < 0.0 ?
      (int16_T)(uint8_T)-(int8_T)(uint8_T)-rtb_Integrator1 : (int16_T)(uint8_T)
      rtb_Integrator1);

    /* DataTypeConversion: '<S5>/Data Type Conversion1' */
    pr2_US_hap_B.DataTypeConversion1 = pr2_US_hap_B.DataTypeConversion_e;
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
    ...
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
       ...
       *  Sum: '<S8>/Add2'
       */
      pr2_US_hap_B.Divide3_d = (pr2_US_hap_B.rout2 -
        pr2_US_hap_P.Constant_Value_d) / pr2_US_hap_P.Constant1_Value_p /
        pr2_US_hap_P.Constant2_Value_e;
    }

    /* Step: '<S8>/Step1' */
    if (tmp_2 < pr2_US_hap_P.Step1_Time_p) {
      tmp_2 = pr2_US_hap_P.Step1_Y0_c;
    } else {
      tmp_2 = pr2_US_hap_P.Wref;
    }

    /* Product: '<S8>/Divide2' incorporates:
     *  Step: '<S8>/Step1'
     */
    pr2_US_hap_B.Divide2 = tmp_2 * pr2_US_hap_B.Divide3_d;
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
      /* Outport: '<Root>/Out1' */
    ...
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
       ...
       *  Store in Global RAM
       */
      tmp_0 = (uint32_T)pr2_US_hap_P.encoder_scale;
      tmp_1 = (uint32_T)(rtb_TSamp - pr2_US_hap_DW.UD_DSTATE);
      sMultiWordMul(&tmp_0, 1, &tmp_1, 1, &pr2_US_hap_B.Gain1.chunks[0U], 2);
       ...
       *  Gain: '<S4>/Gain1'
       */
      sMultiWordMul(&pr2_US_hap_B.Gain1.chunks[0U], 2, &tmp_4.chunks[0U], 2,
                    &pr2_US_hap_B.r2.chunks[0U], 4);
      sMultiWordShr(&pr2_US_hap_B.r2.chunks[0U], 4, 31U,
                    &pr2_US_hap_B.r1.chunks[0U], 4);

      /* DataTypeConversion: '<S4>/Data Type Conversion1' */
      pr2_US_hap_B.DataTypeConversion1_c = MultiWord2sLong
  ...
  if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
    ...
    if (rtmIsMajorTimeStep(pr2_US_hap_M)) {
      ...
      if (pr2_US_hap_P.den_Gd[0] == 0L) {
        ...
        sMultiWord2MultiWord(&denAccum_0.chunks[0U], 2, &pr2_US_hap_B.r4.chunks
                             [0U], 3);
        sMultiWordShl(&pr2_US_hap_B.r4.chunks[0U], 3, 30U,
                      &pr2_US_hap_B.r3.chunks[0U], 3);
        sLong2MultiWord(pr2_US_hap_P.den_Gd[0], &pr2_US_hap_B.r4.chunks[0U], 3);
        sMultiWordDivFloor(&pr2_US_hap_B.r3.chunks[0U], 3,
                           &pr2_US_hap_B.r4.chunks[0U], 3,
                           &pr2_US_hap_B.r.chunks[0U], 4,
                           &pr2_US_hap_B.r5.chunks[0U], 3,
                           &pr2_US_hap_B.r6.chunks[0U], 3, &tmp_3.chunks[0U], 3);
...
void pr2_US_hap_initialize(void)
{
  ...
  /* External mode info */
  pr2_US_hap_M->Sizes.checksums[0] = (1680634554U);
  pr2_US_hap_M->Sizes.checksums[1] = (2929221364U);
  pr2_US_hap_M->Sizes.checksums[2] = (3730619360U);
  pr2_US_hap_M->Sizes.checksums[3] = (3342920561U);
