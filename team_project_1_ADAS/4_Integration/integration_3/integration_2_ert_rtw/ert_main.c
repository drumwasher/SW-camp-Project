...
void rt_OneStep(void)
{
   ...
   */
  integration_2_SetEventsForThisBaseStep(eventFlags);
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(integration_2_M, 0);
  ...
  integration_2_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(integration_2_M->extModeInfo, &rtmGetTFinal
    (integration_2_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(integration_2_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(integration_2_M);
