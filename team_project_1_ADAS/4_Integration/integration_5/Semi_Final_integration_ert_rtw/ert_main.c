...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Semi_Final_integration_M, 0);
  ...
  Semi_Final_integration_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(Semi_Final_integration_M->extModeInfo, &rtmGetTFinal
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Semi_Final_integration_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Semi_Final_integration_M);
  ...
  /* Terminate model */
  Semi_Final_integration_terminate();
