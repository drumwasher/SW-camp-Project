...
void rt_OneStep(void)
{
   ...
   */
  team_project_state_and_cds_adj_integration_SetEventsForThisBaseStep(eventFlags);
      ...
      switch (i)
      {
       case 2 :
        currentTime = (extmodeSimulationTime_T)
          ((team_project_state_and_cds_a_M->Timing.clockTick2 * 10) + 0)
          ;
        team_project_state_and_cds_adj_integration_step2();
       ...
       case 3 :
        currentTime = (extmodeSimulationTime_T)
          ((team_project_state_and_cds_a_M->Timing.clockTick3 * 20) + 0)
          ;
        team_project_state_and_cds_adj_integration_step3();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(team_project_state_and_cds_a_M, 0);
  ...
  team_project_state_and_cds_adj_integration_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(team_project_state_and_cds_a_M->extModeInfo,
    &rtmGetTFinal(team_project_state_and_cds_a_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(team_project_state_and_cds_a_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(team_project_state_and_cds_a_M);
  ...
  /* Terminate model */
  team_project_state_and_cds_adj_integration_terminate();
