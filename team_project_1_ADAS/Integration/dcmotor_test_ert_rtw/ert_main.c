...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) dcmotor_test_M->Timing.clockTick0;
  dcmotor_test_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(dcmotor_test_M->extModeInfo, &rtmGetTFinal
    (dcmotor_test_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(dcmotor_test_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(dcmotor_test_M);
