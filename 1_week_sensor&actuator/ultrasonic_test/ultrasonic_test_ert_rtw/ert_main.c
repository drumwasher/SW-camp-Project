...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) ultrasonic_test_M->Timing.clockTick0;
  ultrasonic_test_step();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(ultrasonic_test_M, 0);
  ...
  ultrasonic_test_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(ultrasonic_test_M->extModeInfo, &rtmGetTFinal
    (ultrasonic_test_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(ultrasonic_test_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(ultrasonic_test_M);
